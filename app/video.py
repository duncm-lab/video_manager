#!/usr/bin/env python3
"""
Interact with video documents and objects
"""
import os
from typing import Union, List
import shutil
from app.database import COLLECTION
from app.project_logging import logger


class Video:

    """CRUD operations for video documents

    Attributes:
        mode (str): when in test mode, alterations to data is prevented
        video_id (str): video id
        processed (bool): flag for if the video has been downloaded
        title (str): video title
        uploader (str): channel name
        upload_date (str): date of video upload
        description (str): description of video
        thumbnail (str): url of thumbnail image
        tags (str or list): A single string or list of strings 
        file_path (str): files location
        processing (bool): flag to indicate this item is being processed
    """

    def __init__(self, 
                 mode: str = '',
                 video_id: str = '', 
                 processed: bool = None, 
                 title: str = '',
                 uploader: str = '', 
                 upload_date: str = '', 
                 description: str = '',
                 thumbnail: str = '', 
                 tags: Union[List, str] = None,
                 file_path: str = ''):

        logger.info('Video object initialised')

        """Constructor method
        Args:
            video_id (str): video id
            processed (bool): processing status
            title (str): video title
            uploader (str): channel
            upload_date (str): upload date
            description (str): video_description
            thumbnail (str): thumbnail url
            file_path (str): location on disk
        """

        self._mode = mode
        self._video_id: str = video_id
        self._processed: bool = processed
        self._title: str = title
        self._uploader: str = uploader
        self._upload_date: str = upload_date
        self._description: str = description
        self._thumbnail: str = thumbnail
        self._tags: Union[List, str] = tags
        self._file_path: str = file_path
        self._processing: bool = False

    def __repr__(self):
        return 'Video(' \
               f'mode=\'{self._mode}\', ' \
               f'video_id=\'{self._video_id}\', ' \
               f'processed={self._processed}, ' \
               f'title=\'{self._title}\', ' \
               f'uploader=\'{self._uploader}\', ' \
               f'upload_date=\'{self._upload_date}\', ' \
               f'description="{self._description}", ' \
               f'thumbnail=\'{self._thumbnail}\', ' \
               f'tags=\'{self._tags}\', ' \
               f'file_path=\'{self._file_path}\')'

    @property
    def mode(self):
        """test or live mode"""
        return self._mode

    @property
    def video_id(self):
        """unique id of video"""
        return self._video_id

    @property
    def processed(self):
        """flag for whether the video has been downloaded"""
        return self._processed

    @processed.setter
    def processed(self, p: bool):
        assert type(p) == bool
        self._processed = p
        logger.info('processed attribute set to %s', str(p))

    @property
    def title(self):
        """title of video"""
        return self._title

    @property
    def uploader(self):
        """User who uploaded video"""
        return self._uploader

    @property
    def upload_date(self):
        """date video uploaded"""
        return self._upload_date

    @property
    def description(self):
        """description of video"""
        return self._description

    @property
    def thumbnail(self):
        return self._thumbnail

    @property
    def tags(self):
        """get tags or return a default value"""
        if self._tags in [None, '', []]:
            return 'unsorted'
        else:
            return self._tags

    @property
    def file_path(self):
        """file path location on disk"""
        return self._file_path

    @file_path.setter
    def file_path(self, path: str):
        self._file_path = path
        logger.info('file_path attribute set to %s', path)

    @property
    def processing(self):
        return self._processing

    @processing.setter
    def processing(self, p: bool):
        assert type(p) == bool
        self._processing = p
        logger.info('processing attribute set to %s', str(p)) 

    def delete_video(self, check: bool = False) -> Union[None, tuple, str]:
        """
        Remove the current object from the database
        along with any files or folders

        Args:
            check (bool): a value to be explicitly set
            in order for the video to be deleted

        Returns:
            (int: 0, str: '') - the document was not found
            (int: 1, str: '') - the document was found and deleted
            but had not path key
            (int: 2, str: '') - the document and path were found
            both were deleted
        """

        if self._mode == 'test':
            return 'Data cannot be deleted in test mode'

        if not check:
            return None

        result = COLLECTION.find_one({'_id': self.video_id},
                                     {'_id': True, 'path': True})

        if not result:
            res = (0, '')
        elif 'path' not in result.keys():
            COLLECTION.delete_one({'_id': self.video_id})
            res = (1, '')
            logger.info('video_id %s deleted - no folder found', self.video_id)
        else:
            COLLECTION.delete_one({'_id': self.video_id})
            path: str = result['path']
            try:
                shutil.rmtree(os.path.split(path)[0], ignore_errors=True)
            except FileNotFoundError as e:
                logger.error(e)
            res = (2, path)
            logger.info('video_id %s deleted - folder %s deleted',
                        self.video_id, path)

        return res

    def save_video(self):
        """save the Video instance to the database"""

        if self._mode == 'test':
            return 'Data cannot be saved in test mode'

        dct = self.__dict__
        doc = {}

        # pop the private modifier of the attribute name before saving
        for k, v in dct.items():
            if k == '_video_id':
                doc['_id'] = v
            elif k == '_processed':
                doc['Processed'] = v
            else:
                dk = k.lstrip('_')
                doc[dk] = v

        COLLECTION.replace_one({'_id': self._video_id}, doc)
        logger.info('%s updated', self._video_id)


class VideoSearch:
    """find videos"""

    @staticmethod
    def query_result_to_video(result: dict, mode='') -> Video:
        """Take a video document and convert it to
        a Video instance
    
        Args:
            result (dict): A document from the database
            mode (str): Set the mode for the video object
    
        Returns:
            Video: an instance of the Video class
        """
    
        vid = result['_id']
        processed = result['Processed']
        title = result['title']
        uploader = result['uploader']
        upload_date = result['upload_date']
        description = result['description']
        thumbnail = result['thumbnail']
        if 'tags' in result.keys():
            tags = result['tags']
        else:
            tags = 'undefined'
    
        ret = Video(mode, vid, processed, title, uploader, upload_date,
                    description, thumbnail, tags)
    
        return ret
    
    @staticmethod
    def get_next_unprocessed(mode='') -> Union[None, Video]:
        res = COLLECTION.find_one({'Processed': False})
    
        if res:
            ret = VideoSearch.query_result_to_video(res, mode)
        else:
            ret = None
    
        return ret
    
    @staticmethod
    def exact_find_video(**kwarg: str) -> Union[Video, None]:
        """ match a value exactly
    
        Args:
            kwarg (dict): kv corresponding to value in db
    
        Returns
            list: empty list for no values found
            list: of Video objects for each match
    
        """
    
        search = COLLECTION.find_one(kwarg)
        if search:
            ret = VideoSearch.query_result_to_video(search)
        else:
            ret = None
    
        return ret
